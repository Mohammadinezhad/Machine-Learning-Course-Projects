...
perceptron_model_batch = Perceptron(max_iter=100000, tol=1e-3, shuffle=False)
perceptron_model_batch.fit(X_train, y_train)

# Make predictions on the test set
y_pred_batch = perceptron_model_batch.predict(X_test)

# Calculate accuracy
accuracy_batch = accuracy_score(y_test, y_pred_batch)
print("Accuracy (Batch):", accuracy_batch)
...
for i, feature_pair in enumerate(combinations(features_list, 2)):
    plot_line_and_points_sklearn(perceptron_model_batch, filtered_data, feature_pair[0], feature_pair[1], axes[i])
